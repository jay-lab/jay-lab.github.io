...
db.colletcion.mapReduce(
  ...
  <reduce>,  // 묶인 정보들끼리 연산하는 함수 명시
  {
    ...
    query: <document>,  // 선택적. map과정 실행 전 필요한 정보를 걸러내는 역할
    sort: <document>,  // 선택적. 어떤 값을 기준으로 정렬할지 설정
    limit: <number>,  // 선택적. 함수에 넣을 도큐먼트의 수 제한
    finalize: <function>,  // 선택적. reduce 과정 끝난 후 실행할 함수 명시
    scope: <document>,  // 선택적. map,reduce,finalize 함수에서 사용 가능한 전역변수설정
    jsMode: <boolean>, // 선택적. map, reduce사이의 정보를 자바스크립트형태로 남길지 설정(기본값 false)
...
2. **그룹핑** : 정해진 도큐먼트를 실제로 묶는 작업. 이는 jsMode값에 따라 처리 위치가 나뉘는데,  
   **false** → 자바스크립트 엔진에서 다시 MongoDB 내부로 정보를 보내서 그곳에서 처리   
...
**앱에 유저가 별점을 주었는데, 1점부터 5점 까지 각각 몇 명이 별점을 주었는지 알아내는 과정**

컬렉션 rating안에는 다음과 같은 데이터들이 들어있다.
...
- map, reduce, finalize 함수에서 입력받은 도큐먼트는 this로 표현된다.
- map함수 다음단계로 도큐먼트를 넘겨주기 위해서는 emit이라는 특별한 함수를 써야한다.
  첫번째 파라미터로는 그룹핑의 기준
  두번째 파라미터로는 다음 단계에 넘겨줄 값을 담는다
...
- reduce 이 후 컬렉션에 저장하고자 할 때 **out**값 형태는 아래 표 참고
...
이와 같이, 리듀스 함수는 단일한 값을 가진 도큐먼트에는 실행되지 않고 여러개의 도큐먼트가 묶인 도큐먼트에 대해서만 실행되기 때문에 의도와 다른 값을 얻게 된 것. 즉, 맵핑 함수만 통과한 도큐먼트와 두 함수 모두 통과한 도큐먼트의 포맷이 달라서 출력된 도큐먼트의 결과도 제각각이 되어버리는 현상

이러한 문제를 해결하기 위해 map함수를 다음과 같이 수정
...
다만 이 경우 의도한 결과가 나올수도 있지만 나오지 않을 수도 있다. 예를들어 내부적으로 rating의 도큐먼트 10개중 4개와 6개를 쪼개어 처리되는 경우 문제가 발생될 가능성이 있다.
...
처리할 데이터가 너무 많을 경우 앞선 도식과 같이 도큐먼트를 분할해서 맵-리듀스 과정을 거치게 된다. 위 도식의 후반부를 보면 중간 결과물과 새로 맵핑한 도큐먼트들을 한 곳에서 넣어서 리듀스 함수에 집어 넣게 된다. 

두번째 도식을 보면, 최종값 4가 나와야하지만 3이 나오는 이유는 아래와 같은 현상때문.
...
즉, 앱에 유저가 별점을 주었는데, 1점부터 5점 까지 각각 몇 명이 별점을 주었는지 알아낼 때의 map & reduce는 다음과 같다.
...
좀 더 응용하여 'rating 컬렉션에서 _id값이 6 이하이면서 _id 값이 가장 큰 4개의 더큐먼트를 rating별로 분류한 다음, 각각의 도큐먼트 개수와 user_id 값들의 합, user_id의 평균을 구하는 방법'
